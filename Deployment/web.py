...
if col1.button("About"):
    ...
    st.sidebar.text("Developed By @Bivan_Banik") 
if col2.button("INFO"):
    st.sidebar.text("Model 1: Random Forest Classifier")
    st.sidebar.text("Test Accuracy : 99%") 
    st.sidebar.text("Model 2: Logistic Regression")
    st.sidebar.text("Test Accuracy : 93%")
if col3.button("HOME"):
    st.sidebar.text("Lets LEarn")
    st.sidebar.text("Grow with ML")
    st.sidebar.text("Go For DATA SCIENCE")
    st.sidebar.text("Built with Streamlit")

# Select Classifiers            
classifier_name = st.sidebar.selectbox("Select Classifier", ("Random Forest", "Logistic Regression"))

def get_model(classifier_name):
    if classifier_name == "Random Forest":
        pickle_in = open("rfmodel1.pkl","rb")
        classifier=pickle.load(pickle_in)
    elif classifier_name == "Logistic Regression":
        pickle_in = open("lrmodel3.pkl","rb")
        classifier=pickle.load(pickle_in)
    return classifier
...
def predict_note_authentication(filename):
    ...
    data = []
    columns_name = []
    #chorus_audio = 'Dynamite - BTS.mp3.wav'
...
def main():
    ...
    st.title("""
        Predict Songs With Two Classifiers
        """)
    ...
    filename = st.file_uploader("Upload Song", type=["wav","mp3"])
